...
session_start();
$userid = $_SESSION['id'];
$sql = "SELECT * FROM tbl_member  WHERE mem_id='$userid' LIMIT 1";
...
$id = $_GET['id'];
      ...
      if($order[0]['or_status'] == 3 || $order[0]['or_status'] == 4) {
...
function Footer()
{
    ...
    // Page number
    $this->Cell(0,10,'Print by '.$user[0]['mem_fullname'],0,0,'L');
	$date = date("d M Y");
...
foreach ($details as $detail) {
  ...
  $pdf->Cell(25,12,$detail['pro_price'].' บาท',1,0,'C',TRUE);
...
$i++;
  ...
  $pdf->Cell(1);
   ...
   $pdf->Cell(180,12,'Total : ' . $total . ' บาท',1,1,'R',TRUE);
  $pdf->Image('../assets/img/notpaid.png', 15, 125, 180) ;
